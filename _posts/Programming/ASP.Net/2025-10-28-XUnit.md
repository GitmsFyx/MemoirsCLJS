...
创建Httpclient测试,不经过浏览器.

在Test项目中导入包.

`Microsoft.AspNetCore.Mvc.Testing` ,提供`WebApplicationFactory`类

`Microsoft.EntityFrameworkCore.InMemory` ,提供内存数据库

`Fizzler`和`Fizzler.Systems.HtmlAgilityPack` ,这两个包都需要才能从HTTP响应中测试HTML DOM.

在Program.cs中暴露class,因为它默认开启顶级语句为隐式
```C#
//Program.cs
...
创建WebApplication工厂,泛型为Program,它通过反射得到里面的设置.
...
public class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            //查找里面单例的类型为DbContextOptions<ApplicationDbContext>的service.
...
创建一个集成测试类  
```c#
//IClassFixture会创建一个CustomWebApplicationFactory类
public class PersonsControllerIntegrationTest:IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        //Testing包内的方法,为你的webapplication创建一个Httpclient
