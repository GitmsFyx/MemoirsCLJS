...
如果一个方法启动了一个异步操作但不返回任何可等待类型（如 `Task` 或`Task<T>`）， 那么它的名称应该以 Begin、Start
...
>* I/O密集型：严重依赖于外部和远程资源且涉及显著等待时间的操作。例子包括写入/读取数据库和调用WebAPI。

>[!TIP]
>async/await 推荐推荐使用在I/O操作（数据库查询、文件读写、网络请求等）中
